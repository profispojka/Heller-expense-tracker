...
| `column2` | Counter-account number | Payee fallback, own-transfer detection, stored as `counterAccount` (categorization identity) |
| `column10` | Counter-account name | `payee` (first choice) |
| `column5` | Variable symbol | Stored as `variableSymbol` (categorization identity) |
...
| `column8` | Transaction type | Stored as `txType`; structural categorization rules (ATM, fee) and a Bayes feature |
...
3. **Categorization** with `Categorizer` (see [01-features.md §4](01-features.md#4-automatic-categorization)),
   fed with payee, note, amount, transaction type, counter-account and variable symbol. A
   category assigned here is stored with `categoryAuto = true` until the user confirms it. A
   counterparty equal to the account owner makes the record a **TRANSFER**.
...
6. **Insert with deduplication:** `INSERT OR IGNORE` against the unique index
   **(accountId, fioTransactionId)**.
   ...
   - A transaction that already exists gets its bank signals (`counterAccount`,
     `variableSymbol`, `txType`) backfilled if they are still empty, which upgrades records
     imported before schema v9.
7. **Balance:**
