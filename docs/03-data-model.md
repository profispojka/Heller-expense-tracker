...
Local **Room** (SQLite) database `heller.db`, **schema version 10**, with five tables. There is no
server and no sync. User settings and Fio connections live in **DataStore**, outside Room.
Categorization learns from the records table itself, so the learning is part of the database
and of every backup.
...
| counterAccount | String? | Counter-account number from the bank (v9). Identity signal for categorization |
| variableSymbol | String? | Variable symbol from the bank (v9) |
| txType | String? | Bank transaction type, Fio `column8` (v9), e.g. "Platba kartou", "Výběr z bankomatu" |
| categoryAuto | Boolean | `true` when the category was assigned by the engine and not yet confirmed by the user (v9). Only records with `false` train the model |
...
| `categorization` | `rules_json` | **Legacy** learned rules from versions before 1.4: JSON object `{merchantKey: categoryId}`. Read-only now, kept so older installs lose nothing |
...
- **No destructive migrations.** Every schema change needs an explicit `Migration`, and without
  ...
  - v8 → v9: `records.counterAccount`, `variableSymbol`, `txType` and `categoryAuto`
    (default `0`, so existing categorized records count as confirmed). The next Fio sync
    backfills the bank signals on already imported records.
  - v9 → v10: the preset category tree shrinks from 82 to 35 rows. Legacy slugs are rewritten
    through `CategoryAliases` in `records.categoryId`, `planned_payments.categoryId`,
    `budgets.categoryGroupIds` and the `parentId` of user-created categories; preset rows are
    then deleted and re-seeded. User-created categories are kept.
